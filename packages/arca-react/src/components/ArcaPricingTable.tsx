...
import type { ArcaAppearance, ArcaClassNames, ArcaLabels, ArcaSubscription, Plan, Tier } from "../types";
import { cn, formatCountdown, formatPrice, humanDuration } from "../utils";
...
export const ArcaPricingTable = ({
  ...
  const maxActivePrice = useMemo(() => {
    ...
    return activeTiers.length > 0 ? Math.max(...activeTiers.map((tier) => Number(tier.price))) : 0;
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className={cn(classNames?.featuresList)} style={{ borderTop: "1px solid var(--arca-border-color)", paddingTop: "24px" }}>
                  {tier.features.map((feature, index) => (
                    <div key={index} className={cn("arca-feat-item", classNames?.featureItem)} style={{ display: "flex", gap: "12px", marginBottom: "16px", alignItems: "flex-start" }}>
